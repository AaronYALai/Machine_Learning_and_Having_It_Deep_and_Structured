...
a_0 = th.shared(0*np.random.randn(neuron).astype(np.float32))
Wi = th.shared(0.01*np.random.randn(48,240).astype(np.float32))
bi = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)

Wh = th.shared(s*np.identity(neuron).astype(np.float32)-a)
Wof = th.shared(0.01*np.random.randn(480,240).astype(np.float32)-a)
Wob = th.shared(0.01*np.random.randn(480,240).astype(np.float32)-a)
bh = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
bof = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
bob = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
"""
W2h = th.shared(s*np.identity(neuron).astype(np.float32)-a)
W2of = th.shared(s*np.concatenate([np.identity(neuron)]*2).astype(np.float32)-a)
W2ob = th.shared(s*np.concatenate([np.identity(neuron)]*2).astype(np.float32)-a)
b2h = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
b2of = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
b2ob = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
"""
W3h = th.shared(s*np.identity(neuron).astype(np.float32)-a)
W3o = th.shared(0.01*np.random.randn(480,48).astype(np.float32))
b3h = th.shared(cons*np.random.randn(neuron).astype(np.float32)-a)
b3o = th.shared(cons*np.random.randn(48).astype(np.float32)-a)

Auxiliary = []; Temp = []
parameters = [Wi,bi,Wh,Wof,Wob,bh,bof,bob,W3h,W3o,b3h,b3o]#W2h,W2of,W2ob,b2h,b2of,b2ob,
for param in parameters:
    Auxiliary.append(th.shared(np.zeros(param.get_value().shape).astype(np.float32)))
    Temp.append(th.shared(np.zeros(param.get_value().shape).astype(np.float32)))
...
a_seq = ifelse(T.lt(bud,1.05), a_out*srng.binomial(size=T.shape(a_out),p=0.8).astype('float32'),a_out)/bud
...
a3_seq = ifelse(T.lt(bud,1.05), a_out*srng.binomial(size=T.shape(a_out),p=0.8).astype('float32'),a_out)/bud
